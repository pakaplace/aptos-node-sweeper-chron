...
test(
  ...
  async () => {
    ...
    const res = await client.getAccountTransactions(account1.address(), {
      start: 0,
    });
    const tx = res.find(
      (e) => e.type === 'user_transaction'
    ) as UserTransaction;
    expect(new HexString(tx.sender).toShortString()).toBe(
      account1.address().toShortString()
    );
